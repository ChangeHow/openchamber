...
- `CodebaseStatusProvider` reads open MRs once per visible project, follows all pages, and shares a branch index with worktree headers and session tooltips. It matches both the source repository ID and exact branch, so a fork's same-named branch cannot claim the badge. The provider refreshes every minute while visible and on foreground resume. Hidden/unmounted sidebars stop polling; scope/runtime changes cancel subsequent pages and discard stale completions. Failed reads retain the last complete links and expose retry rather than clearing badges. MR links use `!number`; GitHub PR discovery remains separate.
