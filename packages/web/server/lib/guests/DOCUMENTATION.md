...
- `DELETE /api/guests/:id` — drops the store row, any agent grant, and socket overrides; stops a running agent. Path-install does not delete the user's folder. Zip and git copies under `{openchamberDataDir}/extensions/` are deleted. Unknown id is 404.
