...
| Helper | `SETTINGS_HELPER_CLASS`, `SETTINGS_DESCRIPTION_CLASS` | Visible option-group descriptions and essential context; other help uses `info` |
...
- Titled option groups: separate sibling groups and preceding controls with `space-y-6` on the parent. Keep related standalone checkboxes in one compact option stack. Group titles sit closer to their own description and options than to the preceding group; use `SettingsControlGroup`'s internal spacing.
